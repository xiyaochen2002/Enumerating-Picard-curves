...
static int tcmp (struct mpoly_term *s, struct mpoly_term *t, int n)
{ 
...
static int tcmp_all (const void *a, const void *b)
    { return tcmp ((struct mpoly_term *)a, (struct mpoly_term *)b, MPOLY_MAX_VARS); }
...
void mpoly_init (mpoly_t mpoly, struct mpoly_term *t, int tcnt, int n)
{
    ...
    s = malloc (tcnt*sizeof(*s));                                               // copy terms in order to sort them
    memcpy (s, t, tcnt*sizeof(*t));
    for ( i = 0 ; i < tcnt ; i++ ) {
        for ( j = 0 ; j < n ; j++ ) if ( s[i].d[j] > mpoly->d[j] ) mpoly->d[j] = s[i].d[j];     // update max degree in each variable
        for ( ; j < MPOLY_MAX_VARS ; j++ ) s[i].d[j] = 0;                           // clear out unused degrees for cleanliness
    }
    ...
    for ( p = mpoly->t[1] ; p < mpoly->e[1] ; p++ ) p->s = (struct mpoly_node *) (mpoly->c + p->s->d);      // c is first element of struct mpoly_node, so this is ok
    mpoly->n = n;
}
