...
static int tcmp (struct mpoly128_term *s, struct mpoly128_term *t, int n)
{
...
static int tcmp_all (const void *a, const void *b)
    { return tcmp ((struct mpoly128_term *)a, (struct mpoly128_term *)b, MPOLY_MAX_VARS); }


void mpoly128_init (mpoly128_t mpoly, struct mpoly128_term *t, int tcnt, int n)
{
    struct mpoly128_term *s;
    struct mpoly128_node *p;
    ...
    memcpy (s, t, tcnt*sizeof(*t));
    for ( i = 0 ; i < tcnt ; i++ ) {
        for ( j = 0 ; j < n ; j++ ) if ( s[i].d[j] > mpoly->d[j] ) mpoly->d[j] = s[i].d[j];
        for ( ; j < MPOLY_MAX_VARS ; j++ ) s[i].d[j] = 0;
    }
    qsort (s, tcnt, sizeof(*s), tcmp_all);
    mpoly->t[n-1] = malloc (tcnt*sizeof(*mpoly->t[n-1]));
    mpoly->e[n-1] = mpoly->t[n-1]+tcnt;
    for ( j = 0 ; j < tcnt ; j++ ) { mpoly->t[n-1][j].c = s[j].c; mpoly->t[n-1][j].d = s[j].d[n-1]; }
    for ( i = n-2 ; i >= 0 ; i-- ) {
    ...
    free (s);
    // the array c effectively replaces the array mpoly->t[0]; it is dense
    mpoly->c = calloc (mpoly->d[0]+1, sizeof(*mpoly->c));
    mpoly->lc = mpoly->c+mpoly->d[0];
    for ( p = mpoly->t[1] ; p < mpoly->e[1] ; p++ ) p->s = (struct mpoly128_node *) (mpoly->c + p->s->d);  // c is first element of struct, so cast is ok
    mpoly->n = n;
}
